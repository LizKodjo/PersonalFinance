...
from models import CurrentUserModel, UserLogin, UserModel
...
async def register(request: Request, newUser: UserLogin = Body(...)) -> UserModel:
    users = request.app.db["users"]
    # hash password
    newUser.password = auth_handler.get_password_hash(newUser.password)
    newUser = newUser.model_dump()
    # check if user already exists
    if (exisiting_username := await users.find_one({"username": newUser["username"]}) is not None):
        raise HTTPException(
...
async def login(request: Request, loginUser: UserLogin = Body(...)) -> str:
    users = request.app.db["users"]
...
@router.get("/me", response_description="Logged in user", response_model=CurrentUserModel)
